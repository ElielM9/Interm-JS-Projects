...
export function initOrderListener() {
  const BTN_START_ORDER = document.querySelector(`#start-order`);

  BTN_START_ORDER.addEventListener(`click`, startOrder);
}

function startOrder() {
  // Obtener datos del formulario
  const inputTableValue = document.querySelector(`#table`).value;
  const inputTableHour = document.querySelector(`#hour`).value;
...
export function addDishToOrder(product) {
  ...
  // Validar que la cantidad sea mayor a 0 y agregar el producto al pedido
  if (quantity > 0) {
    // Si productExists es verdadero, actualiza el arreglo.
    if (productExists) {
      // Actualizar la cantidad
      const orderUpdated = order.map((article) => {
        // Si article.id es igual a id entonces, aricle.quantity se le asigna product.quantity
        if (article.id === id) {
      ...
      // Asignar el nuevo arreglo al objeto dentro del estado
      appState.currentOrder.order = [...orderUpdated];
    } else {
      // Si el articulo no existe se agrega al arreglo
